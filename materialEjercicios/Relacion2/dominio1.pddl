...
;;-------SOLUCIÓN AL EJERCICIO1-------- 
  (:method Case3 ; ni avion ni persona en la ciudad de destino, pero tampoco avion y persona en misma ciudad
	 :precondition (and (at ?p - person ?c1 - city)
...
;-------------EJERCICIO2-------------------
  (:method fuel-insuficiente-fly
	...
	;	(< (+ (+ (total-fuel-used) (* (distance ?c1 ?c2) (slow-burn ?a))) (capacity ?a)) (fuel-limit))
		:tasks(
		     (refuel ?a ?c1)
		     (fly ?a ?c1 ?c2)
		     )
